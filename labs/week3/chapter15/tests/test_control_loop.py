...
def test_verified_in_one_iteration(tmp_path):
    ...
    res = run(task=_task(), policy=_Always([STOP()]), pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ContextManager(budget=10000),
              max_iterations=8, verify=_verify(STATUS_VERIFIED, checks=[{"n": 1}]))
...
def test_verified_requires_verifier(tmp_path):
    ...
    res = run(task=_task(), policy=_Always([STOP()]), pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ContextManager(budget=10000),
              max_iterations=3, verify=_verify(STATUS_FAILED))
    assert res.final_outcome != "VERIFIED"
    assert res.exit_code != 0
...
def test_budget_exhausted_at_cap(tmp_path):
    ...
    res = run(task=_task(), policy=policy, pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ContextManager(budget=10000),
              max_iterations=5, verify=_verify(STATUS_FAILED))
...
def test_noop_policy_stalls(tmp_path):
    ...
    r, controller, pconfig = _setup(tmp_path)
    res = run(task=_task(), policy=_Always([NOOP()]), pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ContextManager(budget=10000),
              max_iterations=6, verify=_verify(STATUS_FAILED))
...
def test_no_compact_overflow_is_stalled_budget(tmp_path):
    ...
    res = run(task=_task(), policy=_Always([STOP()]), pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ctx, max_iterations=8, verify=_verify(STATUS_VERIFIED))
...
def test_consecutive_verifier_errors_terminate_error(tmp_path):
    ...
    res = run(task=_task(), policy=_Always([STOP()]), pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ContextManager(budget=10000),
              max_iterations=8, max_consecutive_errors=2, verify=_verify(STATUS_ERROR))
...
def test_single_error_does_not_terminate(tmp_path):
    ...
    res = run(task=_task(), policy=_Always([STOP()]), pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ContextManager(budget=10000),
              max_iterations=8, max_consecutive_errors=2, verify=v)
    assert res.final_outcome == "VERIFIED"
    assert calls["n"] == 2
...
def test_repeated_denied_cycle_reaches_denied_loop(tmp_path):
    ...
    policy = _Always([ToolCall("edit_file", {"path": "../../escape.py", "op": "append", "new": "x"})])
    res = run(task=_task(), policy=policy, pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ContextManager(budget=10000),
              max_iterations=6, verify=_verify(STATUS_FAILED))
...
def test_deny_does_not_terminate_alone(tmp_path):
    ...
    class _Recover:
        ...
        def select(self, context, observation):
            self.n += 1
            if self.n == 1:
                return [ToolCall("edit_file", {"path": "../../escape.py", "op": "append", "new": "x"})]
            return [STOP()]
    res = run(task=_task(), policy=_Recover(), pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ContextManager(budget=10000),
              max_iterations=6, verify=_verify(STATUS_VERIFIED))
    assert res.final_outcome == "VERIFIED"
...
def test_iteration_rows_carry_full_field_set(tmp_path):
    ...
    res = run(task=_task(), policy=policy, pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ContextManager(budget=10000),
              max_iterations=2, verify=_verify(STATUS_FAILED, checks=[{"n": 1}]))
    for row in res.trajectory["iterations"]:
        assert set(row) == {"iteration", "tool_calls", "tokens", "files_read",
                            "files_modified", "tests_executed", "test_results",
                            "errors", "time_ms", "verdict", "phase"}
...
def test_files_modified_counts_applied_edits(tmp_path):
    r, controller, pconfig = _setup(tmp_path)
    policy = _Always([ToolCall("edit_file", {"path": "repo/config.py", "op": "append", "new": "\nY=2"})])
    res = run(task=_task(), policy=policy, pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ContextManager(budget=10000),
              max_iterations=2, verify=_verify(STATUS_FAILED))
...
def test_never_exceeds_max_iterations(tmp_path):
    ...
    res = run(task=_task(), policy=policy, pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ContextManager(budget=10000),
              max_iterations=4, verify=_verify(STATUS_FAILED))
...
def test_result_type_and_shape(tmp_path):
    ...
    res = run(task=_task(), policy=_Always([STOP()]), pconfig=pconfig, controller=controller,
              sandbox_root=r, context=ContextManager(budget=10000),
              max_iterations=8, verify=_verify(STATUS_VERIFIED))
