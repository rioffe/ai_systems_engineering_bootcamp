...
def test_token_counter_is_synthetic():
    ...
    row = build_row(
        iteration=1, tool_calls=[{"name": "read_file", "args": {"path": "a.py"}}], context_chars=10
    )
...
def test_files_read_distinct_and_in_order():
    row = build_row(
        iteration=2, tool_calls=[], context_chars=0, files_read=["b.py", "a.py", "b.py"]
    )
    assert row["files_read"] == ["b.py", "a.py"]


def test_files_modified_distinct_applied_only():
    row = build_row(iteration=2, tool_calls=[], context_chars=0, files_modified=["a.py", "a.py"])
...
def test_every_field_present_on_every_row():
    ...
    row = build_row(
        iteration=1, tool_calls=[{"name": "list_files", "args": {"path": "."}}], context_chars=50
    )
    expected = {
        "iteration",
        "tool_calls",
...
def test_phase_must_be_in_pinned_vocabulary():
    ...
    for p in PHASES:
        assert build_row(iteration=1, tool_calls=[], context_chars=0, phase=p)["phase"] == p
...
def test_envelope_finalize_and_totals():
    ...
    tr.add_row(
        build_row(
            iteration=1,
            tool_calls=[{"name": "run_shell", "args": {"command": "pytest -q"}}],
            context_chars=100,
            tests_executed=2,
            test_results={"passed": 1, "failed": 1},
            verdict="FAILED",
