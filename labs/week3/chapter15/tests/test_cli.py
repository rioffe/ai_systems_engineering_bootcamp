...
def test_inspect_and_compare_are_offline(tmp_path, capsys):
    ...
    assert main(["compare", "--baseline", str(a), "--current", str(b), "--out", str(report)]) == 0
...
def test_own_source_repo_is_refused(tmp_path):
    ...
    out = tmp_path / "x.json"
    assert main(["run", "--task", "x", "--repo", sandbox.own_source_root(), "--out", str(out)]) == 2
