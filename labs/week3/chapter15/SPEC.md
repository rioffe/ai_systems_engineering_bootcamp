...
>   state."* The agent is a **closed-loop controller** ($A_t \sim \pi_\theta(A \mid C_t, O_t)$, $S_{t+1} = T(S_t, A_t)$, §1); the model supplies the policy, the harness supplies the control
