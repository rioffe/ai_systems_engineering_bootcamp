...
- **Deterministic** (pure, offline, byte-reproducible, no LLM, no network, no embed model):
  ...
  offline**.
- **Probabilistic**: the **Embedder** (`nomic-embed-text`) and the **LLM**
...
tests/               # SPEC §9: 17 files / 136 tests, fully offline
...
`uv run pytest` is the §9 acceptance suite: **17 files / 136 tests, fully offline, ~0.2s**
...
I-009 source scan, CLI exit codes, the availability taxonomy, and the GUI degrade path.
...
- **Four CLI flags are parsed but not wired**: `--llm-rerank` and `--llm-expand` (so
  rerank/expand always use their deterministic doubles), `--stop-on-error`, and
  `--verbose` (the loguru level is fixed, not taken from the namespace).
