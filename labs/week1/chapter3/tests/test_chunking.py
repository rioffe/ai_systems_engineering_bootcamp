...
def _doc(doc_id: str, text: str) -> Document:
    ...
    meta = ChunkMetadata(chunk_id=doc_id, doc_id=doc_id, title=doc_id, section="1.0 Test")
...
def test_contextual_chunker_sets_context_and_embed_prefix():
    ...
    chs = ContextualChunker(
        overlay=FixedChunker(strategy="fixed", chunk_size=200, overlap=0)
    ).chunk(doc)
    # The chunk's context contains the title and section.
    assert any("Document: d3" in c.context or "Section" in (c.context or "") for c in chs)
    # embed_text is the context-prefixed form.
    assert all(
        (c.embed_text or "").startswith("Document")
...
def test_boundary_guard_recovers_split_rule():
    ...
    rule = (
        "Business class airfare refund is $250. "
        "This rule applies only when the policy version is active."
    )
    # A naive 30-char cut would land inside the rule.
    doc = _doc("d5", rule)
    chs = boundary_guard(FixedChunker(strategy="fixed", chunk_size=30, overlap=40).chunk(doc))
