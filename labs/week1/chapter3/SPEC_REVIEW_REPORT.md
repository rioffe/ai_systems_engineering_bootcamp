...
**Recommended resolution** Define each numerator as a **reference-bound, deterministic** check: `relevant_facts in gold_facts` matched by a fixed rule (e.g. the mock's normalized-token overlap; the real judge's emitted `reflected_facts` intersected with `gold_facts`); `relevant_citations = {c : c.chunk_id in relevant_chunks} + {c : c.claim matches a gold_fact}`. State that the *real* judge *emits* `reflected_facts`/`relevant_citations` that are then **intersected with the reference** (never taken at face value), keeping §16's independence intact.
...
**Observation** §0 first calls generation the *one* probabilistic boundary, then R-20 corrects/replaces it with *two* components (Embedder + LLM). Both are *true* — the real path has two Ollama calls — but the language is inconsistent: the §0 framing reads as one boundary and then R-20 says two. The §C-08/C-02 `source-scan` invariant (I-009/T-02) lists *three* modules (`embedding.py`, `model.py`, `judgment.py`) — so the *operational* story is *two interfaces x two roles* = *three modules*.
...
- **Failure-stage enumeration.** `C-12` `failure_stage` in `{retrieval,expansion,reranking,context,generation,judging}` and the state table / E-set agree on *which stage produces which terminal state* **once E-11 is re-pointed** (F-007). After that fix, the *whole* error-attribution story is internally consistent, which is what makes §21's "*where did it fail?" diagnostic trustworthy.
...
- K-01/K-02/K-05: measurable yes, testable yes (T-13/T-14), defined conditions yes (dev box, all mocks, full default dataset). **Pass.**
- Determinism (R-18/I-002): measurable yes (byte-identical), testable yes (T-03/T-07), conditions yes (mock path + fixed seed). **Pass** (modulo F-017's *serialization* ordering and F-016's *seed-surface* scoping).
- Per-case isolation (I-008): measurable yes, testable yes (T-10), conditions yes (a terminal row names one `failure_stage`). **Pass.**
- Offline/full-suite (R-17/I-011/T-14): measurable yes (no Ollama/network/embed model in the suite), testable yes (T-14 / import-scan), conditions yes. **Pass.**
...
- **F-006 — generation numerators are undefined for the *real* judge.** `completeness` ("reflected gold_facts") and `citation_quality` ("relevant citations") are only defined for the *MockJudge* by string/`chunk_id` intersection; for the *real* Ollama judge, *reflected* and *relevant* are undefined (real-vs-real disagreement). Define a *reference-bound matching rule* — `reflected_facts & gold_facts`, `relevant_citations = {c : c.chunk_id in relevant_chunks} + {c : c.claim matches a gold_fact}` — so the real-judgment columns are *independently reproducible* and the `by_capability` diff does not mix a reproducible mock column with an opaque real column.
- **F-014 — `MRR` vs `MRR@k`.** R-11 lists `MRR` *without* an `@k` while `Precis@k`/`Recall@k`/`NDCG@k` carry it; the equation `1/rank(first g in R_k)` *implies* `MRR@k`, but `T-05b` (rank 1) does not pin the rank-`>k` → `0` boundary. State `MRR@k` explicitly and add the rank-`>k` sub-case.
...
- **F-010 (MEDIUM) — token-count convention.** `Context.tokens = sum of est_tokens(doc_text)` over included docs; `truncated` checks the *running sum* before the next append; I-004/I-006 reference that running sum.
