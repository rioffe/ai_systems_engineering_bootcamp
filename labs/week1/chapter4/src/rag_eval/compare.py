...
def compare_artifacts(
    ...
    return {
        ...
        "metrics": metrics,
        "by_category": _category_compare(baseline, current),
        "warnings": (
            ["mixed usage_kind"] if baseline.get("usage_kind") != current.get("usage_kind") else []
        ),
    }
...
def _category_compare(baseline: dict[str, Any], current: dict[str, Any]) -> dict[str, Any]:
    result: dict[str, Any] = {}
    groups = set(baseline.get("aggregate", {}).get("by_category", {})) | set(
        current.get("aggregate", {}).get("by_category", {})
    )
    for group in sorted(groups):
        ...
        for key in METRIC_KEYS:
            result[group][key] = (
                ...
                else (right[key] - left[key] if key in HIGHER_BETTER else left[key] - right[key])
            )
    return result
