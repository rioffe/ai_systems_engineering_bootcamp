...
def _parser() -> argparse.ArgumentParser:
    ...
    check.add_argument("--out", default="dataset_report.json")
    ...
    compare.add_argument("--out", default="compare_report.json")
    compare.add_argument("--force", action="store_true")
    gates = sub.add_parser("gates")
    gates.add_argument("--baseline", required=True)
    gates.add_argument("--current", required=True)
    gates.add_argument("--config", required=True)
    gates.add_argument("--out", default="gate_report.json")
    judge = sub.add_parser("judge-check")
    judge.add_argument("--labels", required=True)
    judge.add_argument("--eval", required=True)
    judge.add_argument("--out", default="judge_check_report.json")
    new_case = sub.add_parser("new-case")
    new_case.add_argument("--trace", required=True)
    new_case.add_argument("--case-id", required=True)
    return parser
    ...
    try:
        ...
        if args.self_check:
            ...
            return (
                1
                if any(
                    any(token in (root / name).read_text() for token in forbidden) for name in core
                )
                else 0
            )
        if args.command == "check":
            ...
            Path(args.out).write_text(
                json.dumps({"ok": not violations, "violations": violations}, sort_keys=True) + "\n"
            )
        ...
        if args.command == "compare":
            ...
            report = compare_artifacts(
                load_artifact(args.baseline, "eval", args.force),
                load_artifact(args.current, "eval", args.force),
                force=args.force,
            )
        ...
        if args.command == "gates":
            ...
            report = compare_artifacts(
                load_artifact(args.baseline, "eval"), load_artifact(args.current, "eval")
            )
