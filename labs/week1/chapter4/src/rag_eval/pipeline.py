...
def run_dataset(
    dataset: Any,
    corpus_dir: str,
    index_flags: dict[str, Any] | None = None,
    ...
    for case in dataset.cases:
        ...
        rows.append(row)
    metric_rows = [
        {**row.get("metrics", {}), "category": row["category"], "difficulty": row.get("difficulty")}
        for row in rows
    ]
    aggregate = aggregate_metrics(
        ...
        if any(case.difficulty for case in dataset.cases)
        else None,
    )
    return {
