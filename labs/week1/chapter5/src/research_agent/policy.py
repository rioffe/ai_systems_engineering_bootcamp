    ...
    def decide(self, state_view: dict[str, Any], tools: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if self.fault == "never_final":
            return {
                "type": "tool_call",
                "tool": "search",
                "arguments": {"query": f"{state_view['goal']} step-{len(observations)}"},
            }
        if self.fault in {"repeat_last_search", "repeat_search"}:
            return {
                "type": "tool_call",
                "tool": "search",
                "arguments": {"query": state_view["goal"]},
            }
        if self.fault == "retrieval_failure" and not self.used_fault:
            ...
            return {
                "type": "tool_call",
                "tool": "retrieve",
                "arguments": {"document_id": "missing-document"},
            }
        if self.fault == "attempt_delete" and not self.used_fault:
            ...
            return {
                "type": "tool_call",
                "tool": "delete_file",
                "arguments": {"path": "/tmp/sentinel"},
            }
        if self.fault == "null_query" and not self.used_fault:
            self.used_fault = True
            return {"type": "tool_call", "tool": "search", "arguments": {"query": None}}
        if self.fault == "retrieval_failure" and any(
            o.get("tool") == "retrieve" for o in observations
        ):
            return {
                "type": "final",
                "report": {
        ...
        searches = [
            o for o in observations if o.get("tool") == "search" and o.get("result") is not None
        ]
        if not searches:
            return {
                "type": "tool_call",
        ...
        if hits and not retrieves:
            return {
                "type": "tool_call",
                "tool": "retrieve",
                "arguments": {"document_id": hits[0]["doc_id"]},
            }
        citations = [
            o["result"]["doc_id"]
            for o in retrieves
            if o.get("result") and isinstance(o["result"], dict)
        ]
        return {
            "type": "final",
            "report": {
                "status": "ok" if citations else "insufficient_evidence",
                "answer": "The available evidence was reviewed."
                if citations
                else "The requested information could not be verified from the available sources.",
                "citations": citations,
                "conflicts": [],
                "caveats": [],
            },
        ...
        response = httpx.post(
            ...
            json={
                ...
                "messages": [
                    ...
                    {"role": "user", "content": json.dumps(state_view, sort_keys=True)},
                ],
                "stream": False,
            },
            timeout=30,
...
def resolve_policy(real: bool, model: str = "qwen3.8:27b-mlx") -> tuple[Policy, str, str, int]:
    if not real:
        return (
            MockPolicy(),
            "DEGRADED_MOCK",
            "[REAL→MOCK] Ollama unreachable; running deterministic mock doubles",
            0,
        )
    try:
        ...
        if model not in names:
            return (
                MockPolicy(),
                "PULL_REQUIRED",
                f"MODEL_MISSING: run 'ollama pull {model}' — or pass --mock",
                4,
            )
