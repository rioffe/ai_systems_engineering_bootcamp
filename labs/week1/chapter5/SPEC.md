...
deliberately ch3-shaped (`search` $\approx$ retrieval, `retrieve` $\approx$ document fetch) so the *only* new
...
| **R-17** | **Observation $\ne$ reasoning** (§7): trace entries SHALL be typed — `action` (the proposed call), `observation` (what the tool returned), `reasoning` (the policy's stated interpretation, when present). A tool result MUST be replayable verbatim from the trace; the policy's conclusions MUST NOT be written into observation records (I-004). |
...
| **I-004** | **Observation $\ne$ reasoning $\ne$ action** (§7): trace step entries are typed (`reasoning`/`action`/`observation`, C-07); tool results are recorded verbatim and replayable; policy interpretation text MUST NOT be written into `observation` entries, and tool output MUST NOT be paraphrased into `action` records. |
...
| **E-10** | Drill's actual `termination_reason` $\ne$ the pinned expectation | drill verdict `pass: false`, exit `1` — terminating for the wrong reason is a failing drill (C-12). |
...
- **T-06b** `TRANSIENT` (injected timeout) → retried with the documented deterministic backoff schedule, $\le$ `max_retries`, then either success or E-12 accounting.
