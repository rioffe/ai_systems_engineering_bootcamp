...
cost, and a **validated** structured output.
...
src/model_playground/
  types.py        # C-01 core types: Message, GenerationParams, Usage, StreamChunk, ModelResponse
  model.py        # C-02 Model interface + OllamaModel + MockModel (+ slow/raising variants)
  ollama.py       # C-03b OllamaClient: thin httpx client over /api/chat, /api/tags
  ...
  worker.py       # C-06 RunWorker(QThread), one per model
  ui.py           # §5 MainWindow + per-model panels + Ollama-unreachable banner
  app.py          # main()
